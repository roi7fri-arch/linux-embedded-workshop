...
void await_setup(int pipe)
{
    ...
    if (read(pipe, buf, 2) != 2)
        printf("Failed to read from pipe: %m\n");
}
...
static void prepare_procfs()
{
    if (mkdir("/proc", 0555) && errno != EEXIST)
        printf("Failed to mkdir /proc: %m\n");

    if (mount("proc", "/proc", "proc", 0, ""))
        printf("Failed to mount proc: %m\n");
}
/********************************************************/
static void prepare_mntns(char *rootfs)
{
    ...
    if (mount(rootfs, mnt, "ext4", MS_BIND, ""))
        printf("Failed to mount %s at %s: %m\n", rootfs, mnt);

    if (chdir(mnt))
        printf("Failed to chdir to rootfs mounted at %s: %m\n", mnt);

    const char *put_old = ".put_old";
    if (mkdir(put_old, 0777) && errno != EEXIST)
        printf("Failed to mkdir put_old %s: %m\n", put_old);

    if (syscall(SYS_pivot_root, ".", put_old))
        printf("Failed to pivot_root from %s to %s: %m\n", rootfs, put_old);

    if (chdir("/"))
        printf("Failed to chdir to new root: %m\n");
 /********************************************************/  
 prepare_procfs();
 /********************************************************/  
    if (umount2(put_old, MNT_DETACH))
        printf("Failed to umount put_old %s: %m\n", put_old);
}
...
static int cmd_exec(void *arg)
{
    ...
    if (prctl(PR_SET_PDEATHSIG, SIGKILL))
        printf("cannot PR_SET_PDEATHSIG for child process: %m\n");
...
/********************************************************/  
    ...
    if (setgid(0) == -1)
        printf("Failed to setgid: %m\n");
    if (setuid(0) == -1)
        printf("Failed to setuid: %m\n");
...
static void write_file(char path[100], char line[100])
{
    ...
    if (fclose(f) != 0) {
        printf("Failed to close file %s: %m\n", path);
    }
}
...
/********************************************************/    
    ...
    if (write(pipe, "OK", 2) != 2)
        printf("Failed to write to pipe: %m");
    if (close(pipe))
        printf("Failed to close pipe: %m");

    if (waitpid(cmd_pid, NULL, 0) == -1)
        printf("Failed to wait pid %d: %m\n", cmd_pid);
