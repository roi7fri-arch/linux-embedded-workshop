...
imx_console_setup(struct console *co, char *options)
{
	...
	pr_info("GAPDBG console_setup enter index=%d options=%s\n",
		co->index, options ? options : "<null>");
	...
	pr_info("GAPDBG console_setup port index=%d sport=%p line=%d\n",
		co->index, sport, sport ? sport->port.line : -1);
	if (sport == NULL)
		return -ENODEV;

	/* For setting the registers, we only need to enable the ipg clock. */
	retval = clk_prepare_enable(sport->clk_ipg);
	pr_info("GAPDBG console_setup after clk_prepare_enable retval=%d\n",
		retval);
	if (retval)
		goto error_console;
	...
	pr_info("GAPDBG console_setup baud=%d parity=%c bits=%d flow=%c\n",
		baud, parity, bits, flow);

	imx_setup_ufcr(sport, 0);

	pr_info("GAPDBG console_setup before uart_set_options\n");
	retval = uart_set_options(&sport->port, co, baud, parity, bits, flow);
	pr_info("GAPDBG console_setup after uart_set_options retval=%d\n",
		retval);
...
error_console:
	pr_info("GAPDBG console_setup exit retval=%d\n", retval);
	return retval;
}
...
static int serial_imx_probe(struct platform_device *pdev)
{
	...
	pr_info("GAPDBG probe enter dev=%s\n", dev_name(&pdev->dev));
	...
	pr_info("GAPDBG probe after probe_dt dev=%s ret=%d line=%d\n",
		dev_name(&pdev->dev), ret, sport->port.line);
	if (ret > 0)
	...
	sport->port.uartclk = clk_get_rate(sport->clk_per);
	pr_info("GAPDBG probe clocks dev=%s uartclk=%u\n",
		dev_name(&pdev->dev), sport->port.uartclk);
	...
	pr_info("GAPDBG probe before uart_add_one_port dev=%s line=%d mapbase=%pa irq=%d\n",
		dev_name(&pdev->dev), sport->port.line, &sport->port.mapbase,
		sport->port.irq);
	ret = uart_add_one_port(&imx_reg, &sport->port);
	pr_info("GAPDBG probe after uart_add_one_port dev=%s line=%d ret=%d\n",
		dev_name(&pdev->dev), sport->port.line, ret);
	return ret;
}
...
static int __init imx_serial_init(void)
{
	...
	pr_info("GAPDBG imx_serial_init enter\n");

	ret = uart_register_driver(&imx_reg);
	pr_info("GAPDBG imx_serial_init after uart_register_driver ret=%d\n",
		ret);
	...
	pr_info("GAPDBG imx_serial_init after platform_driver_register ret=%d\n",
		ret);
	if (ret != 0)
		uart_unregister_driver(&imx_reg);

	pr_info("GAPDBG imx_serial_init exit ret=%d\n", ret);
	return ret;
}
